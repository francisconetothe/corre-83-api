import { Controller, Post, UseInterceptors, UploadedFile, Get, BadRequestException, Body } from '@nestjs/common';
...
export class SettingsController {
  ...
  // Criamos um helper simples para pegar a URL base do ambiente
  private getBaseUrl() {
  ...
  async uploadBanner(@UploadedFile() file: Express.Multer.File) {
    ...
    // 🌍 URL Dinâmica
    const url = `${this.getBaseUrl()}/uploads/banners/${file.filename}`;
    
    await this.prisma.aboutUs.upsert({
      where: { id: 'main-config' },
      update: { imageUrl: url },
      create: { id: 'main-config', content: 'Configurações Gerais', imageUrl: url },
    });
  ...
  async uploadLogo(@UploadedFile() file: Express.Multer.File) {
    ...
    // 🌍 URL Dinâmica
    const url = `${this.getBaseUrl()}/uploads/logos/${file.filename}`;
    
    await this.prisma.aboutUs.upsert({
      where: { id: 'main-config' },
      update: { logoUrl: url },
      create: { id: 'main-config', content: 'Configurações Gerais', logoUrl: url },
    });
  ...
  @UseInterceptors(FileInterceptor('file', {
    storage: diskStorage({
      destination: (req, file, cb) => {
        const uploadPath = join(process.cwd(), 'uploads', 'about');
        if (!existsSync(uploadPath)) mkdirSync(uploadPath, { recursive: true });
        cb(null, uploadPath);
      },
      filename: (req, file, cb) => {
  ...
  async uploadAboutImage(@UploadedFile() file: Express.Multer.File) {
    if (!file) throw new BadRequestException('Imagem de Quem Somos não recebida');

    // 🌍 URL Dinâmica
    const url = `${this.getBaseUrl()}/uploads/about/${file.filename}`;
    
    await this.prisma.aboutUs.upsert({
      where: { id: 'main-config' },
      update: { aboutImageUrl: url },
      create: { id: 'main-config', content: 'Configurações Gerais', aboutImageUrl: url },
    });
  ...
  async updateAboutText(@Body() body: { content: string }) {
    if (!body.content) throw new BadRequestException('O conteúdo não pode estar vazio');
